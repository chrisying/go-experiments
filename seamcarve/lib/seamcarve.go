...
func CarveOne(img image.Image) image.Image {
  ...
  for i := 0; i < height; i++ {
    ...
    for j := 0; j < seamJ; j++ {
      ...
      newImg.Pix[newImg.PixOffset(j, i)] = uint8(r)
      newImg.Pix[newImg.PixOffset(j, i)+1] = uint8(g)
      newImg.Pix[newImg.PixOffset(j, i)+2] = uint8(b)
      newImg.Pix[newImg.PixOffset(j, i)+3] = 255
    }
    for j := seamJ+1; j < width-1; j++ {
      r, g, b, _ := img.At(j, i).RGBA()
      newImg.Pix[newImg.PixOffset(j-1, i)] = uint8(r)
      newImg.Pix[newImg.PixOffset(j-1, i)+1] = uint8(g)
      newImg.Pix[newImg.PixOffset(j-1, i)+2] = uint8(b)
